...
- `INFORMADO` -> organismo confirma -> `CONFIRMADO` + `resultadoFinal = CONDENA_FIRME_PAGADA` (`PCOCNF` se
  ...
  - **Con bloqueantes materiales activos:** no se registra `CIERRA`; acta permanece `ACTIVA` / `ANAL`
    ...
    `../20-application/command-contracts.md`).
...
- La integracion real con Ingresos/Tesoreria es una dependencia externa no bloqueante para DDL (ver `../90-roadmap/current-roadmap.md`).
- Los comprobantes reales de pago quedan como dependencia externa no bloqueante para DDL (ver `../90-roadmap/current-roadmap.md`).
...
Pendiente: reemplazar por implementacion JDBC sin tocar servicios (ver roadmap de DDL/JDBC en `../90-roadmap/current-roadmap.md`).
...
y efectos) vive en
[`../20-application/command-contracts.md`](../20-application/command-contracts.md#pago-aplicado-a-obligacion-anterior---comandos-precondiciones-y-efectos);
