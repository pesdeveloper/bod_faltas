...
/**
 * Gestión del catálogo de medidas preventivas.
 * Solo una versión activa por código en todo momento.
 * Cada nueva versión debe ser mayor a las anteriores.
 */
@Service
public class MedidaPreventivaService {
    ...
    public FalMedidaPreventiva crearPrimeraVersion(
        ...
        if (!versiones.isEmpty())
            throw new PrecondicionVioladaException("Ya existe medida con código=" + codigoNorm + ". Use crearNuevaVersion.");

        Long id = medidaRepo.nextId();
        FalMedidaPreventiva m = new FalMedidaPreventiva(id, codigoNorm, (short) 1, descripcion, LocalDateTime.now(), idUser);
    ...
    public FalMedidaPreventiva crearNuevaVersion(
        ...
        List<FalMedidaPreventiva> versiones = medidaRepo.findVersionesByCodigo(codigoNorm);
        if (versiones.isEmpty())
            throw new PrecondicionVioladaException("No existe medida con código=" + codigoNorm + ". Use crearPrimeraVersion.");
        ...
        Optional<FalMedidaPreventiva> activa = medidaRepo.findActivaByCodigo(codigoNorm);
        activa.ifPresent(a -> {
            a.setSiActiva(false);
            medidaRepo.save(a);
        });
        ...
        Long id = medidaRepo.nextId();
        FalMedidaPreventiva m = new FalMedidaPreventiva(id, codigoNorm, nuevaVersion, descripcion, LocalDateTime.now(), idUser);
        m.setDescripcionDetalle(descripcionDetalle);
        if (idDep != null) m.setDependencia(idDep, verDep);
        m.setSiPuedeBloquearCierre(siPuedeBloquearCierre);
        if (tipoBloqueanteDefault != null) m.setTipoBloqueanteDefault(tipoBloqueanteDefault);
    ...
    public FalMedidaPreventiva desactivar(Long id, String idUser) {
        ...
        if (!m.isSiActiva())
            throw new PrecondicionVioladaException("La medida id=" + id + " ya está inactiva.");
        m.setSiActiva(false);
        return medidaRepo.save(m);
    }
    ...
    public FalArticuloMedidaPreventiva vincularArticulo(
        ...
        if (articuloMedidaRepo.existsActiva(pkId))
            throw new PrecondicionVioladaException("La relación artículo-medida ya está activa: articuloId=" + articuloId + " medidaId=" + medidaPreventivaId);

        medidaRepo.findById(medidaPreventivaId)
                .filter(FalMedidaPreventiva::isSiActiva)
                .orElseThrow(() -> new PrecondicionVioladaException("La medida id=" + medidaPreventivaId + " no existe o está inactiva."));

        FalArticuloMedidaPreventiva rel = new FalArticuloMedidaPreventiva(pkId, siObligatoria, LocalDateTime.now(), idUser);
        return articuloMedidaRepo.save(rel);
    }

    public FalArticuloMedidaPreventiva desvincularArticulo(Long articuloId, Long medidaPreventivaId, String idUser) {
        ArticuloMedidaPreventivaId pkId = new ArticuloMedidaPreventivaId(articuloId, medidaPreventivaId);
        FalArticuloMedidaPreventiva rel = articuloMedidaRepo.findById(pkId)
                .orElseThrow(() -> new PrecondicionVioladaException("No existe relación artículo-medida: articuloId=" + articuloId));
        if (!rel.isSiActiva())
            throw new PrecondicionVioladaException("La relación artículo-medida ya está inactiva.");
    ...
    private void validarCodigoYDescripcion(String codigo, String descripcion) {
        if (codigo == null || codigo.isBlank()) throw new PrecondicionVioladaException("codigo es obligatorio.");
        if (codigo.trim().length() > 12) throw new PrecondicionVioladaException("codigo max 12 caracteres.");
        if (descripcion == null || descripcion.isBlank()) throw new PrecondicionVioladaException("descripcion es obligatoria.");
        if (descripcion.length() > 160) throw new PrecondicionVioladaException("descripcion max 160 caracteres.");
    }
}
