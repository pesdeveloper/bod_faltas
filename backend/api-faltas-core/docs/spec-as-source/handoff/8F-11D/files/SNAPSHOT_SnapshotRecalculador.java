...
public class SnapshotRecalculador {
    ...
    private void derivarBandejaYAccion(
        ...
        // Condena firme con pago confirmado pero acta aun no cerrada (bloqueantes materiales activos).
        // CONDENA_FIRME_PAGADA ya no existe; verificar pago condena CONFIRMADO + ACTIVA.
        ...
        if (acta.getResultadoFinal() == ResultadoFinalActa.CONDENA_FIRME) {
            ...
            // Si hubo pago externo (PAGAPR) pero bloqueantes impiden el cierre: pasar a analisis.
            if (tuvoPagoExterno) {
                snap.setCodBandeja(CodigoBandeja.PENDIENTE_ANALISIS);
                snap.setAccionPendiente(AccionPendiente.NINGUNA);
                return;
