...
public class FalloActaService {
    ...
    public ComandoResultado dictarAbsolutorio(DictarFalloAbsolutorioCommand cmd) {
        ...
        FalActaFallo fallo = new FalActaFallo(idFallo, acta.getId(), TipoFalloActa.ABSOLUTORIO, ahora, ahora, "SYS");
        fallo.setFundamentos(cmd.fundamentos());

        Long idDoc = documentoRepository.nextId();
        FalDocumento doc = new FalDocumento(idDoc, acta.getId(), TipoDocu.ACTO_ADMINISTRATIVO, ahora, "Fallo absolutorio");
    ...
    public ComandoResultado dictarCondenatorio(DictarFalloCondenatorioCommand cmd) {
        ...
        if (cmd.montoCondena() == null || cmd.montoCondena().compareTo(BigDecimal.ZERO) <= 0) {
            throw new PrecondicionVioladaException("El monto de condena debe ser mayor a cero.");
        }
        validarPrecondicionesFallo(acta, cmd.actaId());

        Long idFallo = falloActaRepository.nextId();
        LocalDateTime ahora = LocalDateTime.now();
        FalActaFallo fallo = new FalActaFallo(idFallo, acta.getId(), TipoFalloActa.CONDENATORIO, ahora, ahora, "SYS");
        fallo.setMontoCondena(cmd.montoCondena());
        fallo.setFundamentos(cmd.fundamentos());

        Long idDoc = documentoRepository.nextId();
        FalDocumento doc = new FalDocumento(idDoc, acta.getId(), TipoDocu.ACTO_ADMINISTRATIVO, ahora, "Fallo condenatorio");
        doc.setStorageKey("storage/" + acta.getId() + "/fallo/" + idDoc);
        documentoRepository.guardar(doc);
        ...
        registrarEvento(acta.getId(), TipoEventoActa.FALCON, String.valueOf(idDoc), null, null,
                "Fallo condenatorio dictado. Monto: " + cmd.montoCondena() + ". " + nvl(cmd.observaciones()));
        registrarEvento(acta.getId(), TipoEventoActa.DOCGEN, String.valueOf(idDoc), null, null,
        ...
        return ComandoResultado.de(acta.getId(), String.valueOf(idFallo),
                ...
                "Fallo condenatorio dictado. Monto: " + cmd.montoCondena() + ". Documento pendiente de firma.");
    }
    ...
    private FalActa cargarActaHabilitada(Long actaId) {
        FalActa acta = actaRepository.buscarPorId(actaId).orElseThrow(() -> new ActaNoEncontradaException(actaId));
        if (acta.estaCerrada()) throw new PrecondicionVioladaException("El acta esta cerrada o anulada. No se puede dictar fallo.");
        if (acta.estaParalizada()) throw new PrecondicionVioladaException("El acta esta paralizada. No se puede dictar fallo.");
        return acta;
    }

    private void validarPrecondicionesFallo(FalActa acta, Long actaId) {
        if (acta.getBloqueActual() != BloqueActual.ANAL) {
            throw new PrecondicionVioladaException("Dictar fallo requiere bloque ANAL. Bloque actual: " + acta.getBloqueActual().codigo());
        }
        pagoVoluntarioRepository.buscarPorActa(actaId).ifPresent(pago -> {
            if (pago.getEstadoPagoVoluntario() == EstadoPagoVoluntario.CONFIRMADO) {
                throw new PrecondicionVioladaException("No se puede dictar fallo: existe pago voluntario confirmado.");
            }
        });
        falloActaRepository.buscarActivo(actaId).ifPresent(falloExistente -> {
            throw new PrecondicionVioladaException("Ya existe un fallo activo para este acta. Estado: " + falloExistente.getEstadoFallo());
        });
    }

    private void registrarEvento(Long idActa, TipoEventoActa tipo, String idDocumento, String idNotificacion, String idOperador, String descripcion) {
        int orden = eventoRepository.proximoOrdenLogico(idActa);
        FalActaEvento evento = new FalActaEvento(UUID.randomUUID().toString(), idActa, tipo, LocalDateTime.now(), orden, idDocumento, idNotificacion, idOperador, descripcion, null);
