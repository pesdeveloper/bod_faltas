...
- **fal_acta_transito**: FalActaTransito, ActaTransitoRepository, InMemoryActaTransitoRepository, ActaTransitoService — 1:1 por acta, TRANSITO
- **fal_acta_transito_alcoholemia**: FalActaTransitoAlcoholemia, AtomicLong, lockFinal synchronized, marcarResultadoFinalAtomicamente — N mediciones, una final
- **fal_acta_vehiculo**: FalActaVehiculo, ActaVehiculoService — 1:1, fallback textual, validacion marca/modelo
- **fal_vehiculo_marca**: FalVehiculoMarca, VehiculoMarcaService — unicidad codigo/nombre, baja logica, concurrencia
- **fal_vehiculo_modelo**: FalVehiculoModelo, VehiculoModeloService — unicidad por marca, baja logica, referencias historicas
- **fal_acta_contravencion**: FalActaContravencion, ActaContravencionService — nomenclatura estructurada, pares Suj/Bie, rubro coherente, ambito OTRO
- **fal_acta_sustancias_alimenticias**: FalActaSustanciasAlimenticias, ActaSustanciasAlimenticiasService — rubro coherente, ambito, descripcion larga
- **fal_rubro_version**: FalRubroVersion, RubroVersionService — versionado atomico, una sola version actual por IdRub, hash SHA-256, tests concurrentes
- **fal_acta_medida_preventiva**: FalActaMedidaPreventiva, ActaMedidaPreventivaAplicadaService — relacion articulo-medida, bloqueante atomico, transiciones, rollback logico
...
- **Snapshot**: proyectarSatelites() en SnapshotRecalculador — licenciaProvinciaTxt, licenciaUnidadTxt, nomenclaturaResumen, idBieI, idBieC
- **Seeder**: SatelitesCatalogosSeeder — marcas, modelos, rubros versionados activos/inactivos
- **Tests**: SatelitesCatalogosTest.java — 116 tests nuevos incluyendo CyclicBarrier concurrentes
## Estado al cierre de 8F-11D-R1 (2026-07-06)

> Corrección documental: 62 tablas, 21 FALTA_EN_INMEMORY (29->27 en 8F-11C, 27->21 en 8F-11D+R1). P1 cerrada (SMALLINT 0-4), P2 cerrada (firmeza en fallo). 8F-11D-R1 CERRADO. Build: 1785 tests.

La fuente de verdad de paridad es: `110-matriz-maestra-paridad-mariadb-inmemory.md`
Toda la planificación granular de slices está en la sección 7 de ese documento.
...
- FalActa.tipoActa: String → TipoActa enum con código SMALLINT (D8 cerrada) — valores 1–4
- **ResultadoFinalActa: SMALLINT 0–4 definitivo (P1 CERRADA)** — 0=SIN_RESULTADO_FINAL, 1=PAGO_VOLUNTARIO_PAGADO, 2=ABSUELTO, 3=CONDENA_FIRME, 4=ANULADO
- FalActa.idInspector, idDependencia: String → Long
- Auditoría (fh_alta, id_user_alta, fh_mod, id_user_mod) en FalActa, FalActaFallo, FalActaApelacion, FalGestionExterna
...
### 8F-11C CERRADO — FalPersona y FalPersonaDomicilio (2026-07-05)
...
### 8F-11D CERRADO — Normativa, tarifario, medidas y valorización
...
- Vinculación FalActaFallo.valorizacionId → FalActaValorizacion
...
- **FalRubroVersion** (row_hash, si_version_actual, valid_from, valid_to, synced_at; sincronización externa Informix)
- Guardrails por TipoActa; vínculos exactos marca/modelo/rubro

### 8F-11F — Fallo, firmeza y apelación
- FalActaFallo: valorizacionId, resultadoFallo, fhFirma, fhVtoApelacion, falloReemplazadoId, siFirme, fhFirmeza, origenFirmeza, versionRow, auditoría
- FalActaApelacion: canal, tipo, texto, documentoResolucionId, versionRow, auditoría; falloId → Long
- FalActaApelacionDocumento
- OrigenFirmezaCondena: agregar códigos numéricos (D1 cerrada)
- **FalActaFirmezaCondena: P2 CERRADA (Opción B)** — eliminar FalActaFirmezaCondena y FirmezaCondenaRepository; firmeza en FalActaFallo

### 8F-11G — Paralización, archivo, motivos, observaciones y gestión externa
- FalActaParalizacion (D3 cerrada)
- FalActaArchivo
- **FalMotivoArchivo** (id, descripcion, si_activo; FK desde fal_acta_archivo; CRUD; motivos inactivos válidos históricamente)
- FalObservacion — PAGAPR escribe aquí en lugar de evento.descripcion
- Completar gestión externa
...
- GUARDRAIL: Test-Path docs\spec-as-source desde raíz = FALSE
...
- `ResultadoFinalActa.CONDENA_FIRME_PAGADA (histórico — eliminado del enum en 8F-11B)` agregado
...
- `SnapshotRecalculador`: routing `CONDENA_FIRME + ACTIVA` (histórico: antes CONDENA_FIRME_PAGADA — eliminado en 8F-11B) -> `PENDIENTE_ANALISIS / NINGUNA`- 37 tests nuevos en `GestionExternaTest`- Build limpio: 264/264 tests passing
...
- `FALLO_CONDENATORIO_GESTION_EXTERNA` — **ELIMINADO del enum en 8F-11A-R1** (gestión externa no es un resultado final)
...
- PAGO_VOLUNTARIO_PAGADO (antes PAGO_VOLUNTARIO_CONFIRMADO — renombrado en 8F-11B)
- ABSUELTO
- CONDENA_FIRME (antes CONDENA_FIRME_PAGADA — eliminado en 8F-11B; pago no cambia resultado final)
...
Implementar funcionalmente los dos casos de reingreso desde gestion externa sin intervenciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¯ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â½n de pago:
...
- Ambos no tocan campos PAGAPR (`fechaCierre`, `CERRADA_EXTERNA` (histórico: CONDENA_FIRME_PAGADA → CONDENA_FIRME en 8F-11B)).
...
## Etapa 8 ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â API Funcional Completa Multi-App In-Memory [PLAN MAESTRO]

> **Fuente viva del roadmap de Etapa 8:**
> `backend/api-faltas-core/docs/spec-as-source/100-etapa-8-plan-maestro-api-multi-app.md`
>
> Ese archivo contiene: decisiÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n estratÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©gica, guardrail obligatorio de alineaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n con MariaDB,
> aplicaciones objetivo, diagnÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³stico de 27 mÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³dulos, mapa apps-vs-mÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³dulos, roadmap completo
> 8AÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ8I, dependencias entre slices y criterios de aceptaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n por slice.
>
> **PrÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³ximo slice recomendado: Slice 8A-1 ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Dependencias in-memory.**
> Antes de 8A-1, validar nombres/estructura contra modelo MariaDB.
> No iniciar Etapa 9/JDBC todavÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­a.
...
#### Bloque 8A ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Administracion base
...
#### Bloque 8B ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Talonarios y numeracion
...
#### Bloque 8C ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Documentos y firma
...
#### Bloque 8D ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Observaciones y adjuntos
...
#### Bloque 8E ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Notificaciones y app notificador
...
#### Bloque 8F ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Rodados retenidos y corralo'n
...
#### Bloque 8G ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Portal ciudadano y QR
...
#### Bloque 8H ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Contratos API por aplicacion
...
#### Bloque 8I ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Auditoria API completa in-memory
...
## Etapa 9 ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Persistencia MariaDB/JDBC [FUTURO]
...
## Slice 8A-4 ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Normativas, articulos y dependencia-normativa in-memory (CERRADO 2026-06-30)
...
- FalNormativaFaltas ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â campos exactos de fal_normativa_faltas (id, codigo_norma, version_norma, nombre_norma, descripcion_norma, si_activa, fh_vig_desde, fh_vig_hasta, fh_alta, id_user_alta).
- FalArticuloNormativaFaltas ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â campos exactos de fal_articulo_normativa_faltas (id, normativa_id, codigo_articulo, version_articulo, nombre_articulo, descripcion_articulo, cantidad_unidades, tipo_unidad, si_tiene_pago_voluntario, cantidad_unidades_pago_voluntario, tipo_unidad_pago_voluntario, si_activo, fh_vig_desde, fh_vig_hasta, fh_alta, id_user_alta).
- FalDependenciaNormativa ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â campos exactos de fal_dependencia_normativa (id_dep, ver_dep, normativa_id, si_activa, fh_alta, id_user_alta).
...
- al_tarifario_unidad_faltas ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â pendiente 8C o posterior.
- Valorizaciones / calculos de monto ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â pendiente 8C.
- Catalogos de firma (rol_firma_req, mecanismo_firma_req, tipo_firma, estado_firma_req, requisito_firma) ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â pendiente 8C.
- Alineacion de TipoDocumento y EstadoFirmaDocumento ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â pendiente 8C.
- No se inicio MariaDB/JDBC.
- No se toco Angular.
...
- Comparacion con enums que SI tienen codigos confirmados:
  - ClaseNumeracion: ACTA (1), DOCUMENTO (2) -- catalogo explicito secciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n 9.x
  - TipoTalonario: ELECTRONICO (1), MANUAL_FISICO (2) -- catalogo explicito secciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n 9.3.4
  - AlcanceTalonario: GLOBAL (1), DEPENDENCIA (2), TRANSVERSAL_DOCUMENTO (3) -- catalogo explicito
  - EstadoNumeroTalonario: USADO (1), ANULADO (2), DEVUELTO_SIN_USAR (3), RENDIDO (4), JUSTIFICADO (5) -- catalogo explicito secciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n 9.6
  - MotivoAnulacionTalonario: ERROR_LABRADO (1)...OTRO (5) -- catalogo explicito secciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n 9.6
...
**Fecha:** 2026-07-01
**Objetivo:** Cerrar catÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡logos y base documental mÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­nima necesaria para continuar con 8C sin deuda conceptual.
**Build:** 601/601 tests passing. BUILD SUCCESS.
**Tipo:** Solo documentaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n/modelo. No se implementÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³ Java funcional. No se tocÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³ Angular. No se iniciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³ MariaDB/JDBC.

### Paso 0 - NumPolitica gap resuelto

- Gap detectado en auditorÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­a 8B-7: `NumPolitica` tenÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­a `fhAlta`/`idUserAlta` en Java pero no en el modelo MariaDB.
- ResoluciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n (OpciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n A): Agregados al modelo productivo como auditorÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­a estÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡ndar. Sin cambios Java.
...
| `fal_acta` | Agregado `resultado_firma_infractor SMALLINT NOT NULL` + catÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡logo |
| `fal_inspector_version` | firma_storage_key, firma_hash, fh_firma_registrada ELIMINADOS del modelo (8F-11B) |
| `fal_acta_evidencia` | CatÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡logo `tipo_evid` actualizado: agrega `FIRMA_OLOGRAFA_INFRACTOR` (ID 6) |
...
| `fal_documento_plantilla` | Nueva tabla (SecciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n 5.7) |
| `fal_documento_plantilla_firma_req` | Nueva tabla (SecciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n 5.8) |

### CatÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡logos cerrados

- **`tipo_docu`**: 12 valores (ya existÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­a, confirmado).
- **`estado_docu`**: 7 valores ahora explÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­citos: BORRADOR, EMITIDO, PENDIENTE_FIRMA, FIRMADO, ADJUNTO, ANULADO, REEMPLAZADO.
- **`estado_firma`**: 6 valores (ya existÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­a, confirmado).
...
- SeparaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n explÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­cita: numeraciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n de actas != numeraciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n documental.
- Actas: numeraciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n propia (`nro_acta`), no usan `momento_numeracion_docu`.
- Documentos: numeraciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n por `num_talonario_ambito`, plantilla define momento.
- Firma automÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡tica del inspector en actas (no captura manual por acta).
- Firma ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³lÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³grafa del infractor: opcional, evidencia, no institucional.
...
### Deuda tÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©cnica para Slice 9/JDBC
...
- DDL y seeds de todos los nuevos catÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡logos.

### PrÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³ximos pasos
...
- **Slice posterior**: Implementar `resultado_firma_infractor` y evidencia de firma ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³lÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³grafa.
...
## Micro-slice 8C-4 ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¯ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â½ FalDocumentoFirmaReq snapshot de requisitos de firma [COMPLETADO 2026-07-01]
...
### 8C-5 ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¯ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â½ Enviar documento a firma [PENDIENTE]
...
## Micro-slice 8C-5A ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â Numeracion documental reusable [COMPLETADO 2026-07-01]
...
## 8F-3 ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Mock PDF renderer / generacion final simulada ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â CERRADO
...
- No JDBC, no JPA/Hibernate, no tablas, no seeds de MariaDB.
- No PDF/storage real, no librerÃƒÆ’Ã‚Â­as PDF, no filesystem documental.
...
- No JDBC, no JPA/Hibernate, no tablas, no seeds.
- No PDF/storage real, no librerÃƒÂ­as, no filesystem.
...
**Tests run**: TBD (se actualizarÃ¡ con resultado del build final)
...
- DecisiÃ³n documentada: descuento en ACT-031 usa PCOCNF sin evento propio.
...
### Gaps tÃ©cnicos restantes (pendientes slices futuros)

- FirmaReq completa (workflow de req de firma institucional)
- EmisiÃ³n formal numerada
...
**8F-5 â€” Endpoint dev/test de reset y recreaciÃ³n in-memory.**

Objetivo: endpoint POST /dev/reset que destruye y recrea todos los repositorios in-memory con el dataset mock cargado. Ãštil para tests E2E y desarrollo frontend.
...
**8F-6 â€” Auditoria frontend-ready de endpoints, payloads y flujo demo.**
...
AuditorÃ­a backend-only, frontend-ready, de los 3 endpoints demo.

**Correcciones:**
- Bug corregido en `DevResetControllerIT` test 8: campo `$.totalActas` era incorrecto, debÃ­a ser `$.totalActasMock`.
...
- `DemoContractIT`: 13 tests de contrato frontend-ready que validan:
  ...
  - IDs de navegaciÃ³n en graph (actaId, documentoId, redaccionId).
  - Fields de presentaciÃ³n en dataset (codigo, titulo, bloqueEsperado).
  ...
  - docs/spec-as-source no existe en raÃ­z del repo.
- `108-frontend-ready-demo.md`: documentaciÃ³n viva del flujo demo, payloads, errores y gap list.
...
**No bloqueantes:**
- GAP-1/2/4: Enums tÃ©cnicos sin labels presentacionales.
- GAP-6: actasDemoDisponibles=0 por diseÃ±o.
- GAP-7: No hay DTO global de error (patrÃ³n Map<String,String> no documentado como contrato).

**Slices futuros:**
- GAP-9: PaginaciÃ³n en dataset.
...
- GAP-13: CORS de producciÃ³n por ambiente.

### Guardrails cumplidos

- No JDBC, no MariaDB, no SQL, no JPA/Hibernate.
- No storage real. No archivos fÃ­sicos.
- No endpoint reset fuera de /demo.
- No cambio de semÃ¡ntica funcional de los 31 casos.
- docs/spec-as-source en raÃ­z: False (confirmado).
...
Alternativa: iniciar 8G - App portal ciudadano con los endpoints de consulta pÃºblica documentados en el plan maestro.
...
**BUILD SUCCESS â€” 1486 tests (antes: 1451).**
...
- GET /demo/actas/{codigo} â†’ 200 con shape completo (codigo, titulo, dataset, acta, timeline, documentos, demo, links).
- GET /demo/actas/dataset-funcional â†’ aditivo: cada acta incluye detallePath.
...
**8F-8** - Endpoint GET /demo/health o GET /demo/status (GAP-8) â€” Health/demo-readiness para que el frontend verifique disponibilidad antes de cargar la UI.
...
**BUILD SUCCESS — 1502 tests (antes: 1486, +16).**
...
- GET /demo/health → 200 con shape completo {status, demoReady, fhEjecucion, versionDemo, dataset, documentos, reset, endpoints[], warnings[]}.
...
Demo completamente navegable: /demo/health → /demo/actas/dataset-funcional → /demo/actas/{codigo} → timeline/documentos → /demo/documentos/graph.
...
## Actualización 8F-9-R1 (2026-07-03)

**Slice:** 8F-9-R1 — Cierre DELTA con decisiones de Pablo incorporadas
**Tipo:** Actualización documental. Sin cambios funcionales Java.
**Build:** BUILD SUCCESS — 1502 tests (sin cambios respecto a 8F-8).
...
|---|---|
| D1 — FalActaFirmezaCondena | No tabla separada. Agregar origen_firmeza SMALLINT NULL a fal_acta_fallo |
| D2 — FalPagoVoluntario / FalPagoCondena | Mapear a fal_acta_obligacion_pago con tipo PAGO_VOLUNTARIO / CONDENA |
| D3 — FalActaParalizacion | Implementar in-memory antes de JDBC |
| D4 — Campos JSON documentales | JSON nativo MariaDB 12.3.2 (no TEXT) |
| D5 — FalActaFallo.fundamentos | fundamentos TEXT NULL directamente en fal_acta_fallo |
| D6 — FalPersona / FalPersonaDomicilio | Implementar como entidades separadas in-memory antes de JDBC |
| D7 — FalNotificacion PK | BIGINT AUTO_INCREMENT. Sin UUID alternativo |
| D8 — FalActa.tipoActa | Convertir a enum TipoActa con código SMALLINT |
| D9 — prioridad en plantilla_default | SMALLINT |
...
**Slice 9** puede iniciarse después de que Pablo apruebe el modelo MariaDB actualizado con las nuevas tablas y campos.
