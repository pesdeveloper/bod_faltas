...
- **Resolucion de pago aplicado a obligacion anterior:** no agrega ningun campo a esta tabla. `ResolverPagoObligacionAnteriorCommand` (evento `PAGRES`) nunca crea una obligacion nueva por diferencia: aplica el pago original completo, sin recortar, contra la obligacion vigente existente mediante un movimiento en `fal_acta_pago_movimiento` (ver esa tabla). El saldo resultante se deriva siempre de `fal_acta_economia_proyeccion`; un excedente (pago mayor al saldo pendiente) es informativo, no una obligacion.
...
- **Enums:** `TipoMovimientoPago`, `OrigenMovimiento`, `ClasificacionPago` (incluye `OBLIGACION_ANTERIOR`, codigo 4), `EstadoConciliacionActual`, `OrigenUltimaActualizacion`.
- **Invariante:** vinculo canonico unico `movimientoOrigenId` para reversos y para el movimiento de aplicacion creado al resolver un pago anterior; append-only, sin edicion de movimientos historicos.
- **`ClasificacionPago.OBLIGACION_ANTERIOR`:** un `PAGO_CONFIRMADO` registrado contra una obligacion con `siVigente=false` se clasifica siempre asi (derivado del estado de la obligacion destino, nunca elegido por el actor) y emite `PAGANT` en lugar de `PAGCNF`/`PCOCNF`. No es un valor que el actor pueda declarar contra una obligacion vigente (rechazado con 422). Ver `02-estados-bloques-eventos.md`, seccion "Pago aplicado a obligacion anterior: PAGANT y PAGRES".
- **Campos de recibo `cmtePG`/`prefPG`/`nroPG`:** referencia de recibo real de pago (comercio/prefijo/numero); triple completa o totalmente `NULL` (no parcial); obligatoria (completa) en todo movimiento `PAGO_CONFIRMADO` original (`movimientoOrigenId == null` — 422 si falta o esta parcial); el movimiento de aplicacion (`movimientoOrigenId != null`) puede tener la terna `NULL` (recupera el recibo navegando al original). `origenMovimiento + cmtePG + prefPG + nroPG` es la clave de negocio de un recibo fisico: dos movimientos activos con la misma terna y distinta `referenciaExterna` son un intento de doble registro (409, `MovimientoPagoDuplicadoException`), verificado de forma **atomica** dentro del bloque `synchronized` de `InMemoryPagoMovimientoRepository.append` (no solo con una verificacion previa no atomica). Ver `DECISION_DDL-PAGO-MOV-01`.
- **Resolucion de pago aplicado a obligacion anterior (`PAGRES`), sin tabla propia:** el movimiento de aplicacion creado por `ResolverPagoObligacionAnteriorCommand` es un `FalActaPagoMovimiento` mas: `tipoMovimiento=PAGO_CONFIRMADO`, `clasificacionPago=NORMAL`, `movimientoOrigenId` = id del `PAGANT` original, `importeTotal` igual al importe total del `PAGANT` (sin recortar contra el saldo pendiente), `motivoAplicacionPagoAnterior` (String, nullable) con el motivo normalizado de la resolucion — campo estructurado nuevo, fuente de verdad para idempotencia de motivo (nunca se deriva parseando `descripcionLegible` del evento `PAGRES`). La unicidad de la aplicacion (a lo sumo una por `PAGANT`) es la unicidad de `movimientoOrigenId` en esta misma tabla, no una tabla de resolucion separada. Un reintento compatible devuelve los datos historicos (`idUserAlta`, `motivoAplicacionPagoAnterior`, `fhMovimiento`) de la primera aplicacion, no los de la solicitud de reintento. Ver `03-comandos-precondiciones-efectos.md`.
- **Estado:** LISTO_PARA_DDL.

#### `fal_acta_economia_proyeccion`
- **Campo clave:** `importe_aplicado_total DECIMAL(14,2) NOT NULL DEFAULT 0`.
- **Campos derivados de saldo/excedente:** `saldo_pendiente DECIMAL(14,2) NOT NULL DEFAULT 0` (`= MAX(0, monto - aplicadoNeto)`), `importe_excedente DECIMAL(14,2) NOT NULL DEFAULT 0` (`= MAX(0, aplicadoNeto - monto)`; informativo, nunca una obligacion nueva ni una devolucion), `si_parcialmente_pagada BOOLEAN NOT NULL DEFAULT FALSE` (`aplicadoNeto > 0` y `saldo_pendiente > 0`). `si_cancelada` no es un campo propio: se deriva de `estado_obligacion = CANCELADA_POR_PAGO` (ver `04-snapshot-bandejas-acciones.md`).
...
#### Paridad de concurrencia/idempotencia para resolucion de pago anterior

No existe `fal_acta_pago_resolucion`. Equivalencia entre InMemory vigente y
la garantia productiva requerida en el adapter MariaDB para
`POST /api/faltas/pagos/resolver-pago-anterior`:

| Aspecto | InMemory vigente | MariaDB requerido |
|---|---|---|
| Exclusion | metodo de instancia `synchronized` en `ResolverPagoObligacionAnteriorService.resolver` (monitor `this`, no estatico) | `versionRow` de `FalActaObligacionPago` + transaccion; unicidad fisica de aplicacion (`DECISION_DDL-PAGO-MOV-01`, pendiente) |
...
| Unicidad | verificacion previa en memoria (`PagoMovimientoRepository.findByMovimientoOrigenId`) | constraint unica parcial/condicional sobre `movimiento_origen_id` en `fal_acta_pago_movimiento`, forma final pendiente en `DECISION_DDL-PAGO-MOV-01` |
| Idempotencia | reintento con el mismo `movimientoPagoId`, misma obligacion aplicada y motivo equivalente (comparado contra el campo estructurado `motivoAplicacionPagoAnterior`, nunca parseado de `descripcionLegible`) devuelve el mismo resultado sin crear movimiento ni evento adicional, reportando actor/motivo/fecha **historicos** de la primera aplicacion; distinta obligacion o motivo es conflicto (409) | verificar existencia por `movimiento_origen_id` antes de aplicar; recargar tras conflicto OCC en la obligacion vigente; el resultado reportado debe leerse del registro historico, no de la solicitud de reintento |
...
| `DECISION_DDL-PAGO-MOV-01` | `fal_acta_pago_movimiento`: clave de negocio de recibo real (`origen_movimiento+cmte_pg+pref_pg+nro_pg`, obligatoria y completa en todo `PAGO_CONFIRMADO` original — `movimiento_origen_id IS NULL` —, nulable solo en el movimiento de aplicacion creado por `PAGRES` u otros movimientos internos sin recibo), unicidad de aplicacion por `movimiento_origen_id` para el caso especifico "a lo sumo un movimiento de aplicacion (`PAGO_CONFIRMADO`/`NORMAL`) por `PAGANT` original", columna `motivo_aplicacion_pago_anterior`, FK de `movimiento_origen_id` hacia la propia tabla, y OCC/transaccion del adapter | Para la terna de recibo: (a) `CHECK` de terna completa-o-totalmente-nula (`(cmte_pg IS NULL AND pref_pg IS NULL AND nro_pg IS NULL) OR (cmte_pg IS NOT NULL AND pref_pg IS NOT NULL AND nro_pg IS NOT NULL)`); (b) `CHECK`/regla aplicativa que exija terna no nula cuando `movimiento_origen_id IS NULL AND tipo_movimiento='PAGO_CONFIRMADO'` (recibo obligatorio para todo pago original real; motor SQL puede no soportar subquery en `CHECK`, en cuyo caso la regla queda exclusivamente aplicativa, replicando la validacion ya presente en `PagoMovimientoService.registrar`); (c) `UNIQUE` compuesto simple sobre `(origen_movimiento, cmte_pg, pref_pg, nro_pg)` cuando la terna es no nula (parcial/condicional segun soporte del motor), verificado de forma atomica junto con el insert (equivalente fisico del bloque `synchronized` de `InMemoryPagoMovimientoRepository.append`). Para `movimiento_origen_id`: (d) `UNIQUE` parcial/condicional `WHERE movimiento_origen_id IS NOT NULL AND tipo_movimiento='PAGO_CONFIRMADO' AND clasificacion_pago='NORMAL'` (evita colisionar con el uso no relacionado de `movimiento_origen_id` para enlazar un reverso con su `PAGO_CONFIRMADO` original); (e) `FOREIGN KEY (movimiento_origen_id) REFERENCES fal_acta_pago_movimiento(id)`, autorreferencial; (f) verificacion exclusivamente aplicativa (sin `UNIQUE` fisico) como alternativa a (d), replicando el patron `synchronized` de InMemory con OCC de la obligacion vigente. Columna nueva: `motivo_aplicacion_pago_anterior VARCHAR` (o equivalente), `NULL` por defecto, poblada unicamente en el movimiento de aplicacion (`PAGRES`); es la fuente de verdad de idempotencia de motivo, nunca derivada de `fal_acta_evento.descripcion_legible`. Nota: el reverso y la resolucion comparten hoy la misma columna `movimiento_origen_id` con semantica distinta segun `tipo_movimiento`; ningun caso conocido de dominio reversiona un `PAGANT` ya resuelto (o viceversa), pero esa exclusion mutua no esta modelada como invariante explicita fuera de este DDL | Se cierra al disenar el adapter JDBC de `PagoMovimientoService`/`ResolverPagoObligacionAnteriorService`, segun soporte de indices/`CHECK` parciales del motor/version MariaDB elegida | `110` seccion "Pagos" (`fal_acta_pago_movimiento`), `109` seccion 3, `03-comandos-precondiciones-efectos.md` |
