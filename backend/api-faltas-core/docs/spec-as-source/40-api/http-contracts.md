...
El roadmap de implementacion vigente esta en `../90-roadmap/current-roadmap.md`.
...
GET /demo/health esta cubierto por tests de contrato dedicados (evidencia de conformidad: suite de tests de `api-faltas-core`).
...
| Contrato HTTP integrado | `FlujoCoreIT`, `DemoDeshabilitadoTest`, `DemoActaDetalleContractTest` | Suite completa (evidencia de conformidad: resultado de `mvn test` vigente) |
