...
Proveer un mecanismo seguro de reset y recreacion in-memory para desarrollo, demo y test.

Permite volver el backend a un estado conocido, limpio y reproducible sin reiniciar la aplicacion.

### Alcance dev/test/demo

- Solo disponible cuando `faltas.demo.reset.enabled=true`.
- Sin JDBC, sin MariaDB, sin SQL, sin archivos, sin storage real.
- No borra ni recrea datos reales.
...
Campos: ejecutado, modo, fhReset, repositoriosReseteados, plantillasRecreadas, actasDemoDisponibles, casosDatasetFuncional, errores, repositorios, acciones, advertencias.

### Property de habilitacion

```
faltas.demo.reset.enabled=false   # default en application.yml
```

En tests: `@TestPropertySource(properties = "faltas.demo.reset.enabled=true")`.

Si no habilitada, el endpoint devuelve `404`.

### Repositorios reseteados (24)

Todos los `InMemory*Repository` del paquete `repository.memory` implementan `ResettableInMemoryRepository`:
- reset(): limpia stores y resetea secuencias atomicas a 1.
- nombre(): nombre legible del repositorio.
- size(): total de elementos almacenados.

Repos reseteados: actas, acta-eventos, acta-snapshots, acta-evidencias, apelaciones, bloqueantes-materiales, dependencias, documentos, documento-firmas, documento-firma-reqs, plantillas, plantilla-contenidos, plantilla-defaults, documento-redacciones, fallos, firmantes, firmezas-condena, gestiones-externas, inspectores, normativas, notificaciones, pagos-condena, pagos-voluntarios, talonarios.

### Estrategia de reseed de plantillas

Despues del reset se llama a `PlantillasMockSeeder.seedar(plantillaRepo, contenidoRepo, defaultRepo)`.

Recrea las 8 plantillas mock con IDs fijos (1001-1008), 8 contenidos (2001-2008) y 8 defaults (3001-3008).

Operacion idempotente: `guardar` usa `put()` que sobreescribe.

### Idempotencia

Ejecutar `POST /demo/dev/reset` N veces siempre deja el sistema en el mismo estado:
- stores vacios;
- secuencias atomicas reseteadas a 1;
- 8 plantillas mock presentes;
- sin duplicados ni referencias colgantes.

### Relacion con graph demo

Despues del reset, `GET /demo/documentos/graph` puede ejecutarse inmediatamente.
El servicio crea nuevas actas con `nextId()` (empieza en 1 limpio).

### Relacion con dataset funcional

`DatasetFuncionalDominioCatalog` es estatico (no persiste en repos). Siempre devuelve 31 actas.
`GET /demo/actas/dataset-funcional` funciona sin necesidad de reset.

### Relacion con 8F-6 frontend-ready audit

El reset facilita pruebas E2E desde Angular:
- estado conocido antes de cada flujo de prueba;
- datos demo no acumulados entre sesiones;
- endpoints demo disponibles en estado limpio.

### Restricciones productivas

- No usar `faltas.demo.reset.enabled=true` en produccion.
- No implementar `ResettableInMemoryRepository` en repos JDBC/MariaDB.
- El endpoint no es un endpoint de operacion real.
