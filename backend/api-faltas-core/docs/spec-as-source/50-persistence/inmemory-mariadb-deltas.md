...
| Claves de idempotencia conocidas | `referenciaFirmaExt` (firma), `referenciaExterna` + `origenMovimiento` (pagos), `origenMovimiento` + `cmtePG` + `prefPG` + `nroPG` (recibo real de pago), `loteCodigo` (lote de correo), fecha (dia no computable activo), `movimientoOrigenId` en `fal_acta_pago_movimiento` (unicidad de aplicacion de un pago anterior resuelto) | Deben expresarse como restriccion `UNIQUE` (simple o parcial/condicional segun el motor) | `DECISION_DDL-PAGO-MOV-01` CERRADA: unicidad fisica de `movimientoOrigenId` mediante columna generada nullable + `UNIQUE`; `DECISION_DDL-PERS-01` CERRADA: unicidad de persona por `(tipo_doc, nro_doc)` WHERE no nulos. No existe `fal_acta_pago_resolucion`. La clave de recibo (`origenMovimiento` + `cmtePG` + `prefPG` + `nroPG`) se verifica de forma **atomica** dentro del mismo bloque `synchronized` de `InMemoryPagoMovimientoRepository.append` que ya protege `id` y `origenMovimiento` + `referenciaExterna`. El motivo de la resolucion de un pago anterior se persiste en el propio movimiento de aplicacion (`fal_acta_pago_movimiento.motivo_aplicacion_pago_anterior`, columna nullable), no en el texto del evento `PAGRES`. |
...
| `NO_EXPLICIT_CODE` | El enum no expone `codigo()` — **0 enums en esta categoría** tras el cierre de `DECISION_DDL-ENUM-01` | N/A |

| Aspecto | InMemory vigente | MariaDB objetivo | Delta |
|---|---|---|---|
| Enums `EXPLICIT_NUMERIC_CODE`/`EXPLICIT_STRING_CODE` | Codigo explicito verificado en el enum Java (ver `../00-governance/glossary.md` y [`mariadb-logical-model.md`](mariadb-logical-model.md)) | Columna `SMALLINT` (o `CHAR`/`VARCHAR` para codigo `String`) con el codigo explicito del enum Java | Sin delta pendiente. `ResultadoFinalActa` tiene 10 codigos vigentes (0-9); el codigo 5 (`FALLO_CONDENATORIO_PAGADO`) es LEGACY_RESERVED (`DECISION_DDL-RF-005` CERRADA). |
| Enums promovidos por `DECISION_DDL-ENUM-01` (`EstadoFalloActa`, `EstadoApelacionActa`, `EstadoPagoCondena`, `TipoDiaNoComputable`, `OrigenDiaNoComputable`) | `codigo()` numerico expuesto; verificado en guardrail `SpecAsSourceGuardrailTest.enums_decision_ddl_enum01_tienen_codigo_numerico` | `SMALLINT NOT NULL` con el codigo explicito | Delta resuelto: `DECISION_DDL-ENUM-01` CERRADA. |
...
| OCC | Version en memoria (`versionRow` en el agregado) verificada antes de cada mutacion; conflicto lanza `ConcurrenciaConflictoException` (ver `CONC-STD-001`); implementado en todos los repositorios con OCC: `InMemoryDocumentoRepository`, `InMemoryNotificacionRepository`, `InMemoryGestionExternaRepository`, `InMemoryBloqueanteMaterialRepository`, `InMemoryActaObligacionPagoRepository`, `InMemoryActaFormaPagoRepository`, `InMemoryActaMovimientoRepository`, `InMemoryEconomiaProyeccionRepository`, `InMemoryActaSnapshotRepository` (regenera en lugar de OCC directo) | `version_row` fisico con `UPDATE ... WHERE version_row = :actual`; cero filas afectadas ⇒ `ConcurrenciaConflictoException` | Sin delta de contrato de excepcion; el delta es de mecanismo de deteccion (comparacion en memoria vs. `UPDATE` condicional). |
...
base de entrada para construir el script DDL manual de MariaDB (ver
