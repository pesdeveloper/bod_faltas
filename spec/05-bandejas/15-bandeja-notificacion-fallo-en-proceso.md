# [BANDEJA 15] NOTIFICACIÓN DEL FALLO EN PROCESO

**Estado:** Borrador operativo para validación  
**Última actualización:** 2026-04-07

## Propósito

Definir la bandeja operativa donde quedan los casos cuyo **fallo ya fue emitido para notificación** y cuya **notificación se encuentra en curso**, hasta que exista un resultado suficiente que permita considerar la notificación como fehaciente, o bien tomar una decisión frente a incidencias del diligenciamiento.

Esta bandeja representa el tramo posterior a:

- `14-bandeja-fallos-listos-para-notificar.md`

y previo principalmente a:

- una bandeja de **fallos notificados**, cuando exista notificación fehaciente del fallo
- otras salidas excepcionales o administrativas, si corresponden antes de completar la notificación

---

## Relación con otros documentos

Este documento debe leerse junto con:

- `spec/05-bandejas/00-metodologia-validacion-bandejas.md`
- `spec/05-bandejas/13-bandeja-fallo-pendiente-de-notificacion.md`
- `spec/05-bandejas/14-bandeja-fallos-listos-para-notificar.md`
- documentos canónicos de acto administrativo
- documentos canónicos de documento y firma
- documentos canónicos de notificación
- documentos canónicos de snapshot
- catálogos de:
  - `CanalNotificacion`
  - `EstadoNotificacion`
  - `TipoEventoActa`
  - `EstadoActa`

---

## 1. Nombre de la bandeja

**Bandeja — Notificación del fallo en proceso**

Nombre alternativo conversable con el área:

**Fallos en proceso de notificación**

---

## 2. Objetivo

Permitir operar y controlar los casos en los que la **notificación del fallo ya fue emitida**, pero todavía no existe constancia suficiente de notificación fehaciente ni resultado final del diligenciamiento.

La finalidad de esta bandeja es:

- controlar notificaciones del fallo en curso
- registrar resultados o novedades del diligenciamiento
- detectar falta de acuse, rechazo, devolución o imposibilidad de notificar
- decidir reintentos, correcciones o desvíos
- sacar de esta bandeja los casos que ya tengan notificación fehaciente del fallo

---

## 3. Qué se muestra

En esta bandeja se muestran actas que:

- ya tienen fallo emitido
- ya tuvieron iniciada la notificación del fallo
- tienen una notificación activa, pendiente de resultado o con incidencia abierta
- todavía no cuentan con notificación fehaciente del fallo consolidada

Típicamente se visualizaría, a nivel funcional:

- identificación del acta
- materia o tipo de acta
- infractor o sujeto vinculado
- fecha de emisión del fallo
- fecha de emisión de la notificación del fallo
- canal de notificación
- estado actual de la notificación
- fecha del último movimiento
- observación o incidencia relevante
- si hubo o no acuse
- si hubo rechazo, devolución o imposibilidad
- si está dentro o fuera de plazo operativo esperado
- acción pendiente principal

---

## 4. Qué casos entran

Entran en esta bandeja los casos en que se emitió la notificación del fallo y, desde ese momento, el trámite de notificación quedó operativo y abierto.

Casos típicos de ingreso:

- fallo notificado por canal electrónico y pendiente de resultado operativo suficiente
- fallo enviado por canal postal / diligenciamiento externo y pendiente de recepción de acuse o resultado
- fallo con intento de notificación realizado, pero con novedad aún no resuelta
- fallo cuya notificación requiere seguimiento, reintento o validación manual

No deberían entrar aquí:

- casos cuyo fallo todavía no está listo para notificar
- casos donde la notificación del fallo aún no fue emitida
- casos con notificación fehaciente del fallo ya consolidada
- casos ya cerrados definitivamente
- casos ya enviados a otra estación operativa posterior

---

## 5. Qué se espera resolver aquí

En esta bandeja se espera resolver operativamente si la notificación del fallo:

- fue efectivamente cumplida
- quedó pendiente de acuse o confirmación
- requiere reintento
- presenta una incidencia que obliga a corregir datos, canal o estrategia
- no pudo completarse y exige decisión posterior

Lo central aquí es determinar si el caso:

- sigue en proceso de notificación del fallo
- o ya debe salir a una bandeja de **fallos notificados** por contar con **acuse positivo / notificación fehaciente**

---

## 6. Acciones posibles

### 6.1 Registrar acuse o resultado positivo de notificación del fallo

- **Acción:** registrar que la notificación del fallo fue recibida o cumplida válidamente
- **Evento o proceso que dispara:** recepción de acuse, constancia de entrega, confirmación válida del canal o validación manual equivalente
- **Resultado esperado:** la notificación del fallo queda acreditada como **fehaciente**
- **Destino:** bandeja de **fallos notificados**

**Regla importante:**

- el acuse positivo del fallo **no implica** no implica un paso lineal obligatorio
- primero debe quedar en una bandeja específica de **fallos notificados**
- desde esa bandeja se computa el plazo posterior a la notificación fehaciente del fallo
- la apelación, si existe, es un recurso eventual y no un paso secuencial obligatorio
- si no existe apelación y vence el plazo posterior a la notificación del fallo, el caso puede pasar a gestión externa / apremios, según corresponda
- el cálculo de días debe hacerse desde la **fecha de acuse positivo** de la notificación del fallo

---

### 6.2 Registrar novedad de rechazo, devolución o imposibilidad de notificar

- **Acción:** registrar que la notificación del fallo no pudo concretarse o presentó una incidencia relevante
- **Evento o proceso que dispara:** devolución postal, rechazo del canal, domicilio inválido, imposibilidad material, constancia negativa o incidencia equivalente
- **Resultado esperado:** el caso queda con incidencia explícita y pendiente de decisión operativa
- **Destino:** permanece en esta bandeja hasta que se defina el siguiente paso

---

### 6.3 Reintentar notificación del fallo

- **Acción:** ordenar o iniciar un nuevo intento de notificación del fallo
- **Evento o proceso que dispara:** decisión operativa ante falta de acuse, rechazo, devolución o intento fallido
- **Resultado esperado:** se genera un nuevo ciclo de diligenciamiento o envío por el mismo canal o por otro admitido
- **Destino:** permanece en `15-bandeja-notificacion-fallo-en-proceso.md`

---

### 6.4 Corregir datos operativos para nueva notificación

- **Acción:** corregir datos necesarios para posibilitar un nuevo diligenciamiento
- **Evento o proceso que dispara:** detección de error en domicilio, canal, receptor, datos del fallo o información mínima de notificación
- **Resultado esperado:** el caso queda nuevamente apto para reemitir o reintentar la notificación del fallo
- **Destino:** normalmente permanece en esta bandeja hasta que el nuevo intento quede lanzado o, si la corrección es sustancial, podría requerir reingreso a una bandeja previa

---

### 6.5 Escalar incidencia para decisión administrativa

- **Acción:** remitir el caso para definición interna cuando la notificación del fallo no puede resolverse con una acción operativa simple
- **Evento o proceso que dispara:** reiteración de intentos fallidos, contradicción en constancias, conflicto de datos, situación atípica o criterio jurídico/administrativo pendiente
- **Resultado esperado:** el caso deja de estar en simple seguimiento de notificación y requiere intervención de decisión
- **Destino:** según la incidencia:
  - bandeja o circuito de análisis que corresponda
  - bandeja de paralizadas
  - o el circuito que Dirección de Faltas defina

---

### 6.6 Marcar espera por plazo o ventana de respuesta del diligenciamiento

- **Acción:** dejar el caso en seguimiento hasta el vencimiento del plazo operativo del intento de notificación en curso
- **Evento o proceso que dispara:** notificación ya emitida, sin acuse negativo ni positivo definitivo, dentro del plazo operativo admitido
- **Resultado esperado:** el expediente sigue monitoreado sin salir de la bandeja
- **Destino:** permanece en `15-bandeja-notificacion-fallo-en-proceso.md`

---

### 6.7 Registrar decisión administrativa especial

- **Acción:** aplicar una decisión que altere el recorrido esperado del caso
- **Evento o proceso que dispara:** instrucción de Dirección de Faltas u otra autoridad competente
- **Resultado esperado:** el caso puede cambiar de destino sin pasar necesariamente por el recorrido ordinario esperado
- **Destino:** según la decisión adoptada

---

### 6.8 Paralizar el caso

- **Acción:** suspender o detener el trámite
- **Evento o proceso que dispara:** incidencia relevante, espera externa, conflicto jurídico-administrativo o decisión fundada
- **Resultado esperado:** el expediente sale del curso operativo normal
- **Destino:** bandeja de paralizadas

---

### 6.9 Cerrar el caso

- **Acción:** cerrar administrativamente el expediente desde esta bandeja
- **Evento o proceso que dispara:** decisión fundada que determine que no corresponde continuar con la notificación del fallo por el circuito ordinario
- **Resultado esperado:** el caso se considera finalizado para operación ordinaria
- **Destino:** bandeja de cerradas

---

## 7. Excepciones

### 7.1 Notificación del fallo marcada como emitida pero sin soporte suficiente

Puede ocurrir que el sistema indique que la notificación del fallo fue iniciada, pero falte constancia mínima, dato de canal o respaldo documental suficiente.

En ese caso:

- no debería considerarse una notificación del fallo operativamente sólida
- puede requerirse corrección o revisión interna
- podría corresponder retorno a una etapa previa según la gravedad del problema

---

### 7.2 Incidencias repetidas sin criterio uniforme de salida

Puede ocurrir que existan varios intentos fallidos o constancias contradictorias y no esté claro cuándo insistir, cuándo cambiar canal y cuándo escalar.

Esto debe validarse con Dirección de Faltas para evitar discrecionalidad operativa excesiva.

---

### 7.3 Presentación o novedad relevante durante la notificación del fallo

Puede ocurrir que, mientras la notificación del fallo está en curso, aparezca una presentación, pago, actuación espontánea o novedad externa relevante.

En ese caso, el caso podría requerir:

- reingreso a análisis
- revisión de efectos del fallo
- o cambio de destino operativo

---

### 7.4 Error en el fallo o en la documentación notificada detectado después del envío

Si luego de emitida la notificación del fallo se detecta un error sustancial en el fallo o en la documentación asociada, el caso no debería seguir tratándose solo como una notificación en curso.

Debe evaluarse si corresponde:

- volver a una bandeja previa
- ajustar documentación
- rehacer el fallo si la gravedad lo exige
- o paralizar

---

## 8. Puntos a validar con Dirección de Faltas

1. Si esta bandeja existe realmente como estación operativa diferenciada.

2. Qué eventos concretos consideran suficientes para dar por notificado fehacientemente un fallo, según canal.

3. Qué diferencia práctica hacen entre:
   - notificación del fallo en curso
   - falta de acuse
   - rechazo
   - devolución
   - imposibilidad de notificar
   - notificación fehaciente del fallo

4. Cuándo corresponde reintentar y cuántas veces, antes de escalar o paralizar.

5. Si toda notificación fehaciente del fallo debe pasar primero por una bandeja específica de **fallos notificados**.

6. Desde qué fecha se computa el plazo posterior a la notificación del fallo:
   - fecha de emisión
   - fecha de envío
   - o fecha de acuse positivo

7. Qué incidencias obligan a volver atrás a una bandeja previa.

8. Si desde esta etapa puede existir cierre o paralización directa.

---

## 9. Resultado esperado de validación

Al validar esta bandeja con Dirección de Faltas debería quedar definido:

- si esta estación operativa existe como bandeja real
- qué casos integran realmente el tramo de notificación del fallo en proceso
- qué estados justifican permanencia aquí
- qué se considera notificación fehaciente del fallo
- si el acuse positivo saca siempre al caso de esta bandeja
- cómo nace la bandeja de **fallos notificados**
- desde qué fecha se computa el plazo posterior
- y qué incidencias obligan a corregir, volver atrás, paralizar o cerrar